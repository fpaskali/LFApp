...
intensity data extraction of the pre-defined bands of the analysed LFAs. 

(2) LFA App calibration, which also includes tools for data processing, adding 
additional information and calibration functionality. 

(3) LFA App quantification, which enables quantification of the extracted 
intensity values via loading existing calibration models and 

(4) LFA App Analysis, which includes all the modules mentioned above and enables 
full analysis in one application.
...
the applications can be used in a sequential fashion, the specific steps can be 
...
The image can be rotated or flipped horizontally and vertically via the rotation 
panel on the side. It can also be cropped to a specific size via double click on 
...
strip is six since we are not aware of any lateral flow assay having more than six lines.
...
quantile background correction method; see below. An example can be seen in the 
screenshot below. If you want to change the cropped region you can select "RESET". 
...
The calculated mean and median intensities on the lines in order from top of the 
strip to the bottom of the strip are shown as well.

## 3) Add to data and go back to 1) or proceed with 4)

Clicking "ADD TO DATA" adds the mean and median of the background subtracted 
...
is computed with the app. By clicking "2) MERGE WITH INTENSITY DATA" the two 
...
In the first step, a folder for the analysis results can be selected. If no folder 
is specified, a new folder "LFApp" will be generated in the home directory of the user. 
...
per line, one should set "Number of analytes/colors per line" to the respective number. 
If this number is larger than 1, an additional field will be shown, where one 
should enter the name of the column including the analyte/color information.

<p>![](Opt1Calibration.png){width=100%}</p>

## Optional: reshape data from long to wide

If there is more than one analyte/color per line, one might want to combine both 
information into one calibration model. For such cases it is useful to reshape 
...
generalized additive model (GAM)). A column for concentration values can be 
selected. The concentration values can be logarithmized. You can specify a response 
variable or specify a subset with valid R expressions. An example is visible in 
...
with the help of a calibration model. You can upload an existing model from previous 
calibration analysis and use it to predict the concentrations for the intensity 
values acquired beforehand by clicking "PREDICT". Via the "DOWNLOAD DATA" button 
the data table can be saved as a .csv file containing the intensity values as well as 
